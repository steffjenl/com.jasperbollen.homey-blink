...
class BlinkAPI {
    constructor() {
        ...
        this._region = 'prde';
        this._autToken = 'ERcikyLg1bS1qhjsu-9-fQ';
        this._apiServer = 'rest-prde.immedia-semi.com';
    ...
    _post(endpoint, payload, json = true, ignoreError = false) {
    ...
    login(username, password, uid, notification_key) {
        return new Promise((resolve, reject) => {
            ...
            this._post('/api/v4/account/login', payload).then(response => {
                const result = JSON.parse(response);
                this._region = result.region.tier;
                this._autToken = result.authtoken.authtoken;
    ...
    verfy(pinCode) {
        return new Promise((resolve, reject) => {
            ...
            this._post(`/api/v4/account/${this._account.id}/client/${this._client.id}/pin/verify`, payload, true, true).then(response => {
                const result = JSON.parse(response);
               if (result.valid) {
                   return resolve(result);
               }
    ...
    changed(since = '-999999999-01-01T00:00:00+18:00', page = '1') {
        return new Promise((resolve, reject) => {
            ...
            this._get(`/api/v1/accounts/${this._account.id}/media/changed?since=${since}&page=${page}`, payload).then(response => {
                const result = JSON.parse(response);
                return resolve(result);
            }).catch(error => reject(error));
        });
